...
bool	init_general(t_general *general, char **av, int ac)
{
	...
	// Checker que rien n'est negatif
	...
	error = general->error;
	general->number_of_philo = ft_atol(av[1], &error);
	if (general->number_of_philo > 200 || general->number_of_philo <= 0)
		return (false);
	general->time_to_die = ft_atol(av[2], &error);
	general->time_to_eat = ft_atol(av[3], &error);
	general->time_to_sleep = ft_atol(av[4], &error);
	if (ac == 6)
		general->max_meal = ft_atol(av[5], &error);
	else
		general->max_meal = -1;
